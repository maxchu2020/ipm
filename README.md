...
**到期告警只针对已广播的前缀。** ROA 过期只有在该前缀正在广播时才会造成实际
故障（valid 掉成 invalid、被上游丢弃）；未广播前缀的 ROA 过期不影响现网，混进
告警只会淹没真正要紧的那批。未广播前缀的到期日仍可在 NOT-ANNOUNCED 明细表的
「ROA 到期」列查到，总体结果里也有一行汇总说明。

已广播前缀的到期提醒分两档，都排在报表最前面：

| 档位 | 范围 | 呈现 |
| --- | --- | --- |
| **紧急** | 已过期 + 14 天内 | 报表最顶部的 `!!!` 告警块，邮件标题点名 |
| 预警 | 14~30 天 | 紧急块之后的次级预警段，供排续签计划 |

两档都没有触发时，总体结果里仍会给出到期分布和最早到期时间，功能不会显得是「哑」的。
阈值在 `ipmlib/rov_report.py` 的 `EXPIRY_CRITICAL` / `EXPIRY_WARN`。
...
| 有问题 | `[ipm] ROA/IRR 校验 ⚠ 3 条 ROA 14 天内到期，2 条无授权` |
| 已过期 | `[ipm] ROA/IRR 校验 ⚠ 1 条 ROA 已过期` |
...
./ipm.py scan --retries 5          # 每个探测最多尝试 5 次
...
每个探测最多尝试 **3 次**（`--retries` 可调）。

实测同一条 `/20`，两个维度都会显著影响结果：

| 探测方式 | 尝试次数 | 耗时 | 检出在用 |
| --- | --- | --- | --- |
| 仅 ICMP | 1 | 37 秒 | 50 个 |
| ICMP + TCP | 1 | 315 秒 | 95 个 |
| ICMP + TCP | **3** | 385 秒 | **102 个** |

- **不能只用 ICMP**：近一半主机屏蔽 ICMP，只 ping 会系统性低估。
- **不能只探一次**：单次无响应可能只是丢包，尝试 3 次多检出 7 个（+7.4%），
  代价只是多 22% 时间。

全部 21,504 个地址约 25 分钟，对每日一次完全可接受。
`--retries 1` 可以换回单次探测，此时报表的探测方式一栏会标注
「⚠ 单次探测，丢包会漏报」。
